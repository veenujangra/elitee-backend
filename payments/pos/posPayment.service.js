...
async function deletePayments(userParam) {
    console.log('payments deletePayments calling:');
...
async function createMany(userParam) {
    ...
    // output_result = await Insurer.insertMany(userParam);
    ...
    try {
        ...
        posp_codes = []
        for (const key in userParam) {
            posp_codes.push(userParam[key].posp_code)
        }
        console.log('posp_codes ', posp_codes);

        await Insurer.deleteMany({ posp_code: { $in: posp_codes }, status: "PROCESSING" });
